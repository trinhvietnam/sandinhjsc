	...
	class UniteSettingsUC{
		...
		public function addSection($label,$name=""){
			...
			if(!empty($this->arrSettings) && empty($this->arrSections))
				UniteFunctionsUC::throwError("You should add first section before begin to add settings. (section: $text)");
				
			if(empty($label)) 
				UniteFunctionsUC::throwError("You have some section without text");
		...
		public function mergeSettings(UniteSettingsUC $settings){
			...
			if(empty($arrSectionsNew))
				UniteFunctionsUC::throwError("the section should not be empty");
		...
		public function getArrTextFromAllSettings(){
			...
			foreach($this->arrSections as $section){
				$arrSaps = UniteFunctionsUC::getVal($section, "arrSaps");
				if(empty($arrSaps))
					continue;
