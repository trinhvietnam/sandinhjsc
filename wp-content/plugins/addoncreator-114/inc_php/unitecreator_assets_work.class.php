...
class UniteCreatorAssetsWork extends UniteCreatorAssets{
	...
	public function handleUploadFiles(){
		...
		//get upload path
		$uploadPath = UniteFunctionsUC::getPostVariable("upload_path");
	...
	private function moveFilesFromData($data){
		...
		if(!empty($message)){
			...
			$response["html"] = $html;
			$response["message"] = __("files moved successfully", UNITECREATOR_TEXTDOMAIN);
		}
		return($response);
	}
	...
	private function checkAjaxActions_editor($action, $data){
		...
		switch($action){
			...
			case "assets_save_file":
				...
				HelperUC::ajaxResponseSuccess(__("File Saved", UNITECREATOR_TEXTDOMAIN));
			break;
