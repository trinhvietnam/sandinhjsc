	...
	class UniteFunctionsWPUC_archive{
		...
		public static function getCategoriesHtmlList($catIDs,$strTax = null){
			...
			foreach ( $categories as $category ) {
				...
				if(!empty($link))
					$thelist .= '<a href="' . esc_url( $link ) . '" title="' . esc_attr( sprintf( __( "View all posts in %s", UNITECREATOR_TEXTDOMAIN), $category["name"] ) ) . '" ' . $rel . '>' . $catName.'</a>';
				else
					$thelist .= $catName;
		...
		public static function getArrThumbSizes($includeSmall = false){
			...
			foreach($arrWPSizes as $size){
				...
				switch($size){
					...
					case self::THUMB_SMALL:
						...
						break;
					case "ug_big":
						$title = __("Big", UNITECREATOR_TEXTDOMAIN);
						break;
				}
			...
			$arrSizes[self::THUMB_LARGE] = __("Large (max width 1024)", UNITECREATOR_TEXTDOMAIN);
			$arrSizes[self::THUMB_FULL] = __("Full", UNITECREATOR_TEXTDOMAIN);
		...
		public static function writeSettingLanguageFile($filepath){
			...
			foreach($arrText as $text){
				...
				$str .= "_e(\"$text\",\"".UNITECREATOR_TEXTDOMAIN."\"); \n";				
			}
			$str .= "?>";
